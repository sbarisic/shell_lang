...
public sealed class ValueFormattingTests
{
	[Fact]
	public void DescriptorAwareValueFormatting()
	{
		...
		Assert.Equal("true", Format(engine.CreateValue(core.Bool, true)));
		Assert.Equal("-12", Format(engine.CreateValue(core.Int32, -12)));
		Assert.Equal("1.2345679", Format(engine.CreateValue(core.Float32, 1.2345679F)));
		Assert.Equal("1.2345678901234567", Format(engine.CreateValue(core.Float64, 1.2345678901234567D)));
		var difficulty = engine.Catalog.Enums.Single(type => type.Name == "Difficulty");
		Assert.Equal("Hard", Format(engine.CreateValue(difficulty.Id, Difficulty.Hard)));
		...
		Assert.Equal("[[1, \"two\"]]", Format(nested));
		Assert.Equal("Ok(5)", Format(engine.CreateSuccess(core.Int32, core.Error, engine.CreateValue(core.Int32, 5))));
		Assert.Equal("Ok", Format(engine.CreateVoidSuccess(core.Error)));
		Assert.Equal("Err(Error)", Format(engine.CreateError(core.Int32, core.Error,
			engine.CreateValue(core.Error, new ShellError("must-not-leak")))));

		var output = Execute("format_pair()").Value!;
		Assert.Equal("FormatPair.Output { number: 1, text: \"two\" }", Format(output));
		...
		Assert.Equal("FormattingDerived { derived: \"derived-value\", shadow: \"derived-shadow\", base: \"base-value\" }", derivedText);
		Assert.True(!derivedText.Contains("SECRET", StringComparison.Ordinal));
		Assert.True(!derivedText.Contains("query", StringComparison.Ordinal));
		Assert.Equal(0, queryCalls);
		Assert.Equal("FormattingOpaque", Format(engine.CreateValue(opaqueType.Id, new FormattingOpaque("OPAQUE-SECRET"))));

		var self = new FormattingNode("self");
		self.Next = self;
		Assert.Equal("FormattingNode { name: \"self\", next: <cycle: FormattingNode>, path: \"\" }",
		...
		Assert.True(shallowCycle.Contains("next: <cycle: FormattingNode>", StringComparison.Ordinal));
		Assert.True(!shallowCycle.Contains("<max-depth:", StringComparison.Ordinal));
		...
		Assert.True(cycle.Contains("next: FormattingNode { name: \"second\", next: <cycle: FormattingNode>", StringComparison.Ordinal));
		...
		Assert.Equal(0, Count(shared, "<cycle:"));
		Assert.Equal(2, Count(shared, "FormattingNode {"));
		Assert.True(shared.Contains("path: \"[0]\"", StringComparison.Ordinal));
		Assert.True(shared.Contains("path: \"[1]\"", StringComparison.Ordinal));
		...
		Assert.True(defaultDepth.Contains("<max-depth: FormattingNode>", StringComparison.Ordinal));
		var shallow = engine.FormatValue(engine.CreateValue(nodeType.Id, chain), session, new ValueFormatOptions { MaxDepth = 1 });
		Assert.True(shallow.Contains("next: <max-depth: FormattingNode>", StringComparison.Ordinal));
		Assert.Throws<ArgumentOutOfRangeException>(() => engine.FormatValue(derived, session, new ValueFormatOptions { MaxDepth = 0 }));
		...
		Assert.Equal("FormattingFailures { healthy: 7, throws: <unavailable: String>, null_value: <unavailable: String>, wrong_type: <unavailable: String>, adapter_invalid: <unavailable: FormattingToggle>, session_value: 42 }", failures);
		Assert.True(!failures.Contains("getter secret", StringComparison.Ordinal));

		session.SetBinding("formatted", derived);
		printed.Clear();
		var printResult = Execute("formatted -> print");
		Assert.Equal(ExecutionStatus.Completed, printResult.Status);
		Assert.True(printResult.Value is null);
		Assert.Equal(derivedText, printed.Single());
		printed.Clear();
		Execute("\"Hello world\" -> print");
		Assert.Equal("\"Hello world\"", printed.Single());
		Assert.Equal(0, queryCalls);
