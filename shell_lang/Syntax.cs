...
internal sealed class Lexer
{
    ...
    public Lexer(string source, List<CompilationDiagnostic> diagnostics) { _source = source; _diagnostics = diagnostics; }
    ...
    public IReadOnlyList<Token> Lex()
    {
        ...
        {
            var start = _position; var c = _source[_position];
            if (c is ' ' or '\t' or '\f') { _position++; continue; }
            if (c == '\r' || c == '\n')
            {
                if (c == '\r' && Peek(1) == '\n') _position += 2; else _position++;
                result.Add(Make(TokenKind.NewLine, start, _position)); continue;
            }
            if (c == '#') { while (_position < _source.Length && _source[_position] is not '\r' and not '\n') _position++; continue; }
            if (char.IsLetter(c) || c == '_')
            {
                _position++;
                while (char.IsLetterOrDigit(Peek(0)) || Peek(0) == '_') _position++;
                var text = _source[start.._position];
                result.Add(new Token(text switch { "true" => TokenKind.True, "false" => TokenKind.False, _ => TokenKind.Identifier },
            ...
            {
                _position++;
                while (char.IsDigit(Peek(0))) _position++;
                var fractional = false;
                if (Peek(0) == '.' && char.IsDigit(Peek(1)))
                {
                    fractional = true; _position++;
                    while (char.IsDigit(Peek(0))) _position++;
                }
                if (Peek(0) is 'e' or 'E')
                {
                    fractional = true; _position++;
                    if (Peek(0) is '+' or '-') _position++;
                    if (!char.IsDigit(Peek(0))) AddDiagnostic("SL1002", "An exponent requires digits.", start, _position);
                    while (char.IsDigit(Peek(0))) _position++;
                }
                var text = _source[start.._position];
            ...
            if (c == '"')
            {
                _position++; var value = new StringBuilder(); var terminated = false;
                while (_position < _source.Length)
                {
                    c = _source[_position++];
                    ...
                    if (escaped == 'u')
                    {
                        var hexStart = _position; var valid = true;
                        for (var i = 0; i < 4; i++) if (!Uri.IsHexDigit(Peek(i))) valid = false;
                        if (!valid) AddDiagnostic("SL1003", "A Unicode escape requires four hexadecimal digits.", _position - 2, Math.Min(_source.Length, _position + 4));
                        else { value.Append((char)int.Parse(_source.AsSpan(hexStart, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture)); _position += 4; }
                    }
                    else
                    {
                        value.Append(escaped switch { 'n' => '\n', 'r' => '\r', 't' => '\t', '"' => '"', '\\' => '\\', _ => escaped });
                        if (escaped is not ('n' or 'r' or 't' or '"' or '\\')) AddDiagnostic("SL1003", $"Unknown escape '\\{escaped}'.", _position - 2, _position);
                    }
                }
                if (!terminated) AddDiagnostic("SL1004", "Unterminated string literal.", start, _position);
                result.Add(new Token(TokenKind.String, _source[start.._position], value.ToString(), Span(start, _position))); continue;
            }
            var kind = c switch
            {
                '(' => TokenKind.OpenParen, ')' => TokenKind.CloseParen, '[' => TokenKind.OpenBracket, ']' => TokenKind.CloseBracket,
            ...
            var width = 1;
            if (c == '-' && Peek(1) == '>') { kind = TokenKind.Arrow; width = 2; }
            else if (c == '<' && Peek(1) == '-') { kind = TokenKind.InputArrow; width = 2; }
            else if (c == '=' && Peek(1) == '=') { kind = TokenKind.EqualEqual; width = 2; }
            else if (c == '!' && Peek(1) == '=') { kind = TokenKind.BangEqual; width = 2; }
            else if (c == '<' && Peek(1) == '=') { kind = TokenKind.LessEqual; width = 2; }
            else if (c == '>' && Peek(1) == '=') { kind = TokenKind.GreaterEqual; width = 2; }
            else if (c == '&' && Peek(1) == '&') { kind = TokenKind.AndAnd; width = 2; }
            else if (c == '|' && Peek(1) == '|') { kind = TokenKind.OrOr; width = 2; }
            else if (c == '=') kind = TokenKind.Assign;
            else if (c == '!') kind = TokenKind.Bang;
            else if (c == '<') kind = TokenKind.Less;
            else if (c == '>') kind = TokenKind.Greater;
            else if (c == '-') kind = TokenKind.Minus;
            _position += width;
            if (kind == TokenKind.Bad) AddDiagnostic("SL1001", $"Unexpected character '{c}'.", start, _position);
...
internal enum InvocationEntryKind { Positional, NamedArgument, ExplicitInput }
...
internal sealed class Parser
{
    ...
    { _source = source; _tokens = tokens; _diagnostics = diagnostics; }

    public ScriptSyntax ParseScript()
    {
        var statements = new List<StatementSyntax>(); SkipTerminators();
        while (Current.Kind != TokenKind.End)
        {
            ...
            {
                var name = Next(); Next(); SkipOperandNewlines();
            ...
            if (Current.Kind is not (TokenKind.NewLine or TokenKind.Semicolon or TokenKind.End))
            {
                ...
                while (Current.Kind is not (TokenKind.NewLine or TokenKind.Semicolon or TokenKind.End)) Next();
            }
            SkipTerminators();
        }
    ...
    {
        var source = ParseBinary(0); var stages = new List<ExpressionSyntax>();
        while (true)
        {
            SkipNewlinesBefore(TokenKind.Arrow);
            if (Current.Kind != TokenKind.Arrow) break;
            Next(); SkipOperandNewlines();
            var stage = ParseStage(); stages.Add(stage);
        }
    ...
    private ExpressionSyntax ParseStage()
    {
        ...
        while (true)
        {
            ...
            if (Current.Kind != TokenKind.Dot) break;
    ...
    private ExpressionSyntax ParseBinary(int parentPrecedence)
    {
        ...
        {
            var op = Next(); SkipOperandNewlines(); var operand = ParseBinary(unary);
            left = new UnarySyntax(op.Kind, operand, Bounds(op.Span.Offset, operand.Span.End));
        }
        else left = ParsePostfix();
        while (true)
        {
            var precedence = BinaryPrecedence(Current.Kind);
            if (precedence == 0 || precedence <= parentPrecedence) break;
            var op = Next(); SkipOperandNewlines(); var right = ParseBinary(precedence);
    ...
    {
        Next(); SkipOperandNewlines(); var name = Match(TokenKind.Identifier, "Expected a member name.");
        IReadOnlyList<InvocationEntrySyntax>? args = null; var end = name.Span.End;
        if (Current.Kind == TokenKind.OpenParen) { args = ParseEntries(TokenKind.CloseParen, out end); }
    ...
    private ExpressionSyntax ParsePrimary()
    {
        ...
        { Next(); return new LiteralSyntax(token); }
        if (token.Kind == TokenKind.Identifier)
            return Peek(1).Kind == TokenKind.OpenParen ? ParseInvocation() : new NameSyntax(Next().Text, token.Span);
        if (token.Kind == TokenKind.OpenParen)
        {
            var open = Next(); _delimiterDepth++; SkipDelimiterNewlines(); var expression = ParseExpression(); SkipDelimiterNewlines();
            var close = Match(TokenKind.CloseParen, "Expected ')'."); _delimiterDepth--;
            return new ParenthesizedSyntax(expression, Bounds(open.Span.Offset, close.Span.End));
        }
        if (token.Kind == TokenKind.OpenBracket)
        {
            var open = Next(); _delimiterDepth++; var items = new List<ExpressionSyntax>(); SkipDelimiterNewlines();
            while (Current.Kind is not (TokenKind.CloseBracket or TokenKind.End))
            {
                items.Add(ParseExpression()); SkipDelimiterNewlines();
                if (Current.Kind != TokenKind.Comma) break; Next(); SkipDelimiterNewlines();
            }
            var close = Match(TokenKind.CloseBracket, "Expected ']'."); _delimiterDepth--;
            return new ArraySyntax(items, Bounds(open.Span.Offset, close.Span.End));
        }
        if (token.Kind == TokenKind.Dot)
        {
            var dot = Next(); var name = Match(TokenKind.Identifier, "Expected a contextual member name.");
            IReadOnlyList<InvocationEntrySyntax>? args = null; var end = name.Span.End;
            if (Current.Kind == TokenKind.OpenParen) args = ParseEntries(TokenKind.CloseParen, out end);
            return new ContextMemberSyntax(name.Text, args, Bounds(dot.Span.Offset, end));
        }
        Error("SL1103", $"Expected an expression, found '{token.Text}'.", token.Span); Next();
    ...
    {
        Match(TokenKind.OpenParen, "Expected '('."); _delimiterDepth++; SkipDelimiterNewlines();
        var entries = new List<InvocationEntrySyntax>();
        while (Current.Kind != closeKind && Current.Kind != TokenKind.End)
        {
            var start = Current.Span.Offset; InvocationEntryKind kind; string? name = null;
            if (Current.Kind == TokenKind.Identifier && Peek(1).Kind is TokenKind.Colon or TokenKind.InputArrow)
            {
                name = Next().Text; kind = Current.Kind == TokenKind.Colon ? InvocationEntryKind.NamedArgument : InvocationEntryKind.ExplicitInput;
                Next(); SkipOperandNewlines();
            }
            else kind = InvocationEntryKind.Positional;
            var expression = ParseExpression(); entries.Add(new(kind, name, expression, Bounds(start, expression.Span.End)));
            SkipDelimiterNewlines();
            if (Current.Kind != TokenKind.Comma) break; Next(); SkipDelimiterNewlines();
        }
        var close = Match(closeKind, $"Expected '{(closeKind == TokenKind.CloseParen ? ")" : "]")}'.");
        _delimiterDepth--; end = close.Span.End; return entries;
    }

    private void SkipTerminators() { while (Current.Kind is TokenKind.NewLine or TokenKind.Semicolon) Next(); }
    private void SkipDelimiterNewlines() { if (_delimiterDepth > 0) while (Current.Kind == TokenKind.NewLine) Next(); }
    private void SkipOperandNewlines() { while (Current.Kind == TokenKind.NewLine) Next(); }
    private void SkipNewlinesBefore(TokenKind kind)
    {
        var offset = 0; while (Peek(offset).Kind == TokenKind.NewLine) offset++;
        if (Peek(offset).Kind == kind) for (var i = 0; i < offset; i++) Next();
    }
    private Token Match(TokenKind kind, string message)
    {
        if (Current.Kind == kind) return Next();
        Error("SL1104", message, Current.Span); return new Token(kind, string.Empty, null, Current.Span with { Length = 0 });
    }
    private Token Next() { var current = Current; if (_position < _tokens.Count - 1) _position++; return current; }
    ...
    private static int BinaryPrecedence(TokenKind kind) => kind switch
    {
        ...
        TokenKind.AndAnd => 2, TokenKind.OrOr => 1, _ => 0
    };
}
