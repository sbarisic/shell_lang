...
public sealed class CoreTypeCatalog
{
    ...
    public ShellTypeId Any { get; internal init; }
    public ShellTypeId Void { get; internal init; }
    public ShellTypeId Bool { get; internal init; }
    public ShellTypeId Int32 { get; internal init; }
    public ShellTypeId Int64 { get; internal init; }
    public ShellTypeId UInt32 { get; internal init; }
    public ShellTypeId UInt64 { get; internal init; }
    public ShellTypeId Float32 { get; internal init; }
    public ShellTypeId Float64 { get; internal init; }
    public ShellTypeId String { get; internal init; }
    public ShellTypeId Error { get; internal init; }
    public ShellTypeId EmptyCollectionError { get; internal init; }
}

internal enum ShellTypeKind { Core, Host, Enum, Error, Array, Result, OutputRecord, ReservedStream }

internal sealed class TypeEntry
{
    public required ShellTypeId Id { get; init; }
    public required string Name { get; set; }
    public required string Description { get; init; }
    public required ShellTypeKind Kind { get; init; }
    public Type? ClrType { get; init; }
    public ValueAdapter? Adapter { get; init; }
    ...
    public EqualityDescriptor? Equality { get; init; }
    public OrderingDescriptor? Ordering { get; init; }
    public ShellTypeId? ElementType { get; init; }
    public ShellTypeId? SuccessType { get; init; }
    public ShellTypeId? ErrorType { get; init; }
    public IReadOnlyDictionary<string, ShellTypeId>? OutputFields { get; init; }
...
public sealed partial class ShellEngine
{
    ...
    private SymbolId NextSymbol(object value)
    {
        ...
        _symbols.Add(id, value); return id;
    }
    ...
    private void InitializeCoreTypes()
    {
            ...
            ShellTypeId? baseType = null, EqualityDescriptor? equality = null, OrderingDescriptor? ordering = null) where T : notnull
        {
            ...
            {
                Id = id, Name = name, Description = $"Core {name} type.", Kind = kind,
                ClrType = typeof(T), Adapter = adapter,
                Bases = baseType is null ? [] : [baseType.Value], Equality = equality, Ordering = ordering
            };
            _typeEntries.Add(id, entry); _typesByName.Add(name, entry); return id;
        }
        ...
        {
            Any = any, Void = voidType, Bool = boolean, Int32 = int32, Int64 = int64,
            UInt32 = uint32, UInt64 = uint64, Float32 = float32, Float64 = float64,
            String = str, Error = error, EmptyCollectionError = empty
        };
        foreach (var name in IntrinsicNames)
        {
            ...
            _symbols[id] = descriptor; Intrinsics.Add(name, descriptor);
        }
    }

    private void AddEntry(TypeEntry entry)
    {
        _typeEntries.Add(entry.Id, entry); _typesByName.Add(entry.Name, entry);
    }
    ...
    {
        if (element == Core.Void) throw new ArgumentException("Array<Void> is not valid.", nameof(element));
        if (_arrayTypes.TryGetValue(element, out var existing)) return existing;
        var id = IdentitySource.NextType();
        var entry = new TypeEntry
        {
            Id = id, Name = $"Array<#{element.Value}>", Description = "Immutable array.",
            Kind = ShellTypeKind.Array, ClrType = typeof(ShellArrayValue), ElementType = element
        };
        _typeEntries.Add(id, entry); _arrayTypes.Add(element, id); return id;
    }

    internal ShellTypeId ResultOf(ShellTypeId success, ShellTypeId error)
    {
        _ = GetTypeEntry(success);
        if (!IsAssignable(error, Core.Error)) throw new ArgumentException("Result error type must derive from Error.", nameof(error));
        if (_resultTypes.TryGetValue((success, error), out var existing)) return existing;
        var id = IdentitySource.NextType();
        var entry = new TypeEntry
        {
            Id = id, Name = $"Result<{GetTypeEntry(success).Name},{GetTypeEntry(error).Name}>", Description = "Typed result.",
            Kind = ShellTypeKind.Result, ClrType = typeof(ShellResultValue), SuccessType = success, ErrorType = error
        };
        _typeEntries.Add(id, entry); _resultTypes.Add((success, error), id); return id;
    }

    internal bool IsAssignable(ShellTypeId actual, ShellTypeId expected)
    {
        if (actual == expected || expected == Core.Any) return actual != Core.Void;
        var a = GetTypeEntry(actual); var e = GetTypeEntry(expected);
        ...
        while (queue.TryDequeue(out var current))
        {
            ...
            foreach (var parent in GetTypeEntry(current).Bases) queue.Enqueue(parent);
        }
        return false;
    }
    ...
    internal ShellTypeId CommonError(ShellTypeId left, ShellTypeId right)
    {
        ...
        {
            ancestors.Add(current); var bases = GetTypeEntry(current).Bases;
            if (bases.Count == 0) break; current = bases[0];
        }
        for (var current = right; ;)
        {
            if (ancestors.Contains(current)) return current;
            var bases = GetTypeEntry(current).Bases;
            if (bases.Count == 0) return Core.Error; current = bases[0];
        }
    }
    ...
    {
        member = null; query = null;
        var queue = new Queue<ShellTypeId>(); queue.Enqueue(receiver);
        ...
        while (queue.TryDequeue(out var id))
        {
            ...
            if (member is not null || query is not null) return entry;
            foreach (var parent in entry.Bases) queue.Enqueue(parent);
        }
        return null;
    }
    ...
    private List<HostingDiagnostic> Validate(DescriptorSet set)
    {
        ...
        {
            if (!IdentifierPattern.IsMatch(name)) d.Add(new("SL3001", $"Invalid {kind} name '{name}'."));
        }
        foreach (var item in set.Types.Cast<object>().Concat(set.Enums).Concat(set.Errors))
        {
            var name = item switch { TypeDescriptor x => x.Name, EnumTypeDescriptor x => x.Name, ErrorTypeDescriptor x => x.Name, _ => "" };
            Name(name, "type");
            if (name == "Stream" || name.StartsWith("Stream_", StringComparison.Ordinal)) d.Add(new("SL3016", "Stream<T> is reserved and cannot be registered in 0.1."));
            if (_typesByName.ContainsKey(name) || !newTypeNames.Add(name)) d.Add(new("SL3002", $"Duplicate type '{name}'."));
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(type.Description)) d.Add(new("SL3003", $"Type '{type.Name}' needs a description."));
            if (type.Adapter is null || type.ClrType != type.Adapter.ClrType) d.Add(new("SL3004", $"Type '{type.Name}' has an invalid adapter."));
            foreach (var b in type.DirectBases) if (!available.Contains(b)) d.Add(new("SL3005", $"Type '{type.Name}' has unknown base {b}."));
            ValidateLocalNames(type.Name, type.Members.Select(x => x.Name).Concat(type.Queries.Select(x => x.Name)), d);
            foreach (var member in type.Members)
            {
                Name(member.Name, "member");
                if (string.IsNullOrWhiteSpace(member.Description)) d.Add(new("SL3003", $"Member '{type.Name}.{member.Name}' needs a description."));
                if (!IsKnownTypeReference(member.ValueType, available)) d.Add(new("SL3005", $"Member '{type.Name}.{member.Name}' has unknown type."));
            }
            foreach (var query in type.Queries)
            {
                Name(query.Name, "query");
                if (string.IsNullOrWhiteSpace(query.Description)) d.Add(new("SL3003", $"Query '{type.Name}.{query.Name}' needs a description."));
                if (!IsKnownTypeReference(query.OutputType, available)) d.Add(new("SL3005", $"Query '{type.Name}.{query.Name}' has unknown output type."));
                ValidateLocalNames($"{type.Name}.{query.Name}", query.Arguments.Select(x => x.Name), d);
                foreach (var argument in query.Arguments)
                {
                    Name(argument.Name, "argument");
                    if (!IsKnownTypeReference(argument.Type, available)) d.Add(new("SL3005", $"Query argument '{type.Name}.{query.Name}.{argument.Name}' has unknown type."));
        ...
        {
            if (type.Members.Count == 0) d.Add(new("SL3006", $"Enum '{type.Name}' has no members."));
        ...
        foreach (var error in set.Errors)
        {
            ...
            if (!knownError) d.Add(new("SL3017", $"Error '{error.Name}' must have one error base rooted at Error."));
        }
        ...
        foreach (var command in set.Commands)
        {
            ...
            if (!commandNames.Add(command.Name) || IntrinsicNames.Contains(command.Name)) d.Add(new("SL3007", $"Duplicate or reserved command '{command.Name}'."));
            if (string.IsNullOrWhiteSpace(command.Description)) d.Add(new("SL3003", $"Command '{command.Name}' needs a description."));
            if (command.Inputs.Count(x => x.IsDefault) > 1) d.Add(new("SL3008", $"Command '{command.Name}' has more than one default input."));
            if (command.Outputs.Count(x => x.IsDefault) > 1) d.Add(new("SL3009", $"Command '{command.Name}' has more than one default output."));
            ...
            foreach (var port in command.Inputs) { Name(port.Name, "input"); if (string.IsNullOrWhiteSpace(port.Description)) d.Add(new("SL3003", $"Input '{command.Name}.{port.Name}' needs a description.")); }
            foreach (var argument in command.Arguments) { Name(argument.Name, "argument"); if (string.IsNullOrWhiteSpace(argument.Description)) d.Add(new("SL3003", $"Argument '{command.Name}.{argument.Name}' needs a description.")); }
            foreach (var output in command.Outputs) { Name(output.Name, "output"); if (string.IsNullOrWhiteSpace(output.Description)) d.Add(new("SL3003", $"Output '{command.Name}.{output.Name}' needs a description.")); }
            foreach (var item in command.Inputs.Select(x => x.Type).Concat(command.Arguments.Select(x => x.Type)).Concat(command.Outputs.Select(x => x.Type)))
                if (!IsKnownTypeReference(item, available)) d.Add(new("SL3005", $"Command '{command.Name}' references unknown type {item}."));
            ...
            if (command.ErrorType is { } et && !(newErrorIds.Contains(et) || (_typeEntries.TryGetValue(et, out var errorEntry) && errorEntry.Kind == ShellTypeKind.Error))) d.Add(new("SL3017", $"Command '{command.Name}' has a non-error ErrorType."));
            foreach (var fault in command.RuntimeFaults)
                if (!RuntimeFaults.ContainsKey(fault.Value) && !set.RuntimeFaults.Any(x => x.Code == fault)) d.Add(new("SL3011", $"Command '{command.Name}' references unknown fault {fault}."));
        }
        foreach (var global in set.Globals)
        {
            Name(global.Name, "global");
            if (Globals.ContainsKey(global.Name) || set.Globals.Count(x => x.Name == global.Name) > 1) d.Add(new("SL3012", $"Duplicate global '{global.Name}'."));
            if (!IsKnownTypeReference(global.Type, available)) d.Add(new("SL3005", $"Global '{global.Name}' has unknown type."));
            if (string.IsNullOrWhiteSpace(global.Description)) d.Add(new("SL3003", $"Global '{global.Name}' needs a description."));
        }
        foreach (var fault in set.RuntimeFaults)
        {
            if (!FaultPattern.IsMatch(fault.Code.Value) || fault.Code.Value.StartsWith("SL", StringComparison.Ordinal)) d.Add(new("SL3013", $"Invalid runtime fault code '{fault.Code}'."));
            Name(fault.Name, "runtime fault");
            if (RuntimeFaults.ContainsKey(fault.Code.Value) || set.RuntimeFaults.Count(x => x.Code == fault.Code) > 1) d.Add(new("SL3014", $"Duplicate runtime fault '{fault.Code}'."));
            if (RuntimeFaults.Values.Any(x => x.Name == fault.Name) || set.RuntimeFaults.Count(x => x.Name == fault.Name) > 1) d.Add(new("SL3014", $"Duplicate runtime fault name '{fault.Name}'."));
            if (string.IsNullOrWhiteSpace(fault.Description)) d.Add(new("SL3003", $"Runtime fault '{fault.Code}' needs a description."));
        }
        return d;
    }
    ...
    private static void ValidateLocalNames(string owner, IEnumerable<string> names, List<HostingDiagnostic> diagnostics)
    {
        ...
        foreach (var name in names)
            if (!seen.Add(name)) diagnostics.Add(new("SL3015", $"Duplicate local name '{owner}.{name}'."));
    }

    private bool IsKnownTypeReference(ShellTypeId type, HashSet<ShellTypeId> available)
    {
        if (!available.Contains(type)) return false;
        if (!_typeEntries.TryGetValue(type, out var entry)) return true;
    ...
    private void ValidateTypeCycles(IReadOnlyList<TypeDescriptor> types, List<HostingDiagnostic> diagnostics)
    {
        ...
        var visiting = new HashSet<ShellTypeId>(); var visited = new HashSet<ShellTypeId>();
        bool Visit(ShellTypeId id)
        {
            if (visiting.Contains(id)) return true;
            if (!visited.Add(id) || !map.TryGetValue(id, out var bases)) return false;
            visiting.Add(id); foreach (var parent in bases) if (Visit(parent)) return true; visiting.Remove(id); return false;
        }
        foreach (var type in types) if (Visit(type.Id)) { diagnostics.Add(new("SL3018", $"Nominal type graph containing '{type.Name}' is cyclic.")); break; }
    }
    ...
    private void ValidateErrorCycles(IReadOnlyList<ErrorTypeDescriptor> errors, List<HostingDiagnostic> diagnostics)
    {
        ...
        {
            var seen = new HashSet<ShellTypeId>(); var current = error.Id;
            while (map.TryGetValue(current, out var parent))
            {
                if (!seen.Add(current)) { diagnostics.Add(new("SL3019", $"Error chain containing '{error.Name}' is cyclic.")); return; }
                current = parent;
            }
        }
    }
    ...
    public RegistrationResult Register(DescriptorSet descriptors)
    {
        ...
        if (diagnostics.Count > 0) return new RegistrationResult(diagnostics);
        foreach (var type in descriptors.Types)
        {
            type.SymbolId = NextSymbol(type);
            foreach (var member in type.Members) { member.ReceiverType = type.Id; member.Id = NextSymbol(member); }
            foreach (var query in type.Queries) { query.ReceiverType = type.Id; query.Id = NextSymbol(query); }
            AddEntry(new TypeEntry { Id = type.Id, Name = type.Name, Description = type.Description, Kind = ShellTypeKind.Host,
                ClrType = type.ClrType, Adapter = type.Adapter, Bases = type.DirectBases, Members = type.Members, Queries = type.Queries,
                Equality = type.Equality, Ordering = type.Ordering });
            Types.Add(type);
        ...
        {
            type.SymbolId = NextSymbol(type);
            AddEntry(new TypeEntry { Id = type.Id, Name = type.Name, Description = type.Description, Kind = ShellTypeKind.Enum,
                ClrType = type.ClrType, Adapter = type.Adapter, EnumMembers = type.Members, Ordering = type.Ordering,
        ...
        foreach (var error in descriptors.Errors)
        {
            ...
            AddEntry(new TypeEntry { Id = error.Id, Name = error.Name, Description = error.Description, Kind = ShellTypeKind.Error,
                ClrType = error.ClrType, Adapter = error.Adapter, Bases = [error.BaseType] }); Errors.Add(error);
            RefreshConstructedTypeNames(error.Id, error.Name);
        }
        foreach (var fault in descriptors.RuntimeFaults) RuntimeFaults.Add(fault.Code.Value, fault);
        foreach (var global in descriptors.Globals) { global.Id = NextSymbol(global); Globals.Add(global.Name, global); }
        foreach (var command in descriptors.Commands)
        {
            ...
            if (command.Outputs.Count > 1)
            {
                ...
                _typeEntries.Add(id, new TypeEntry { Id = id, Name = $"{ToPascal(command.Name)}.Output", Description = $"Outputs of {command.Name}.",
                    Kind = ShellTypeKind.OutputRecord, ClrType = typeof(ShellOutputRecordValue),
    ...
    {
        if (_arrayTypes.TryGetValue(type, out var array)) _typeEntries[array].Name = $"Array<{name}>";
    }
