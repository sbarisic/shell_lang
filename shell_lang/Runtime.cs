    ...
    private EvalOutcome ContextWhere(BoundIntrinsicOperation operation, ShellValue primary, ShellArrayValue array, IReadOnlyList<int> path)
    {
        ...
        try
        {
            for (var i = 0; i < array.Items.Count; i++)
            {
                ...
                {
                    var contextualResult = (ShellResultValue)predicateValue.Value;
                    if (contextualResult is ShellResultValue.Error error)
                    {
                        var frames = error.Frames.Concat([new ErrorContextFrame("array", i.ToString(), operation.Span, i)]).ToArray();
        ...
        return _engine.GetTypeEntry(operation.DirectOutput).Kind == ShellTypeKind.Result
            ? EvalOutcome.Success(new ShellValue(operation.DirectOutput, new ShellResultValue.Success(filtered)))
            : EvalOutcome.Success(filtered);
    }
    ...
    private EvalOutcome ContextSort(BoundIntrinsicOperation operation, ShellValue primary, ShellArrayValue array, IReadOnlyList<int> path)
    {
        ...
        try
        {
            for (var i = 0; i < array.Items.Count; i++)
            {
                ...
                {
                    var contextualResult = (ShellResultValue)keyValue.Value;
                    if (contextualResult is ShellResultValue.Error error)
                    {
                        var frames = error.Frames.Concat([new ErrorContextFrame("array", i.ToString(), operation.Span, i)]).ToArray();
                        return EvalOutcome.Success(new ShellValue(operation.DirectOutput, new ShellResultValue.Error(error.Value, frames)));
                    }
                    keyValue = ((ShellResultValue.Success)contextualResult).Value;
                }
        ...
        try
        {
            ...
            return _engine.GetTypeEntry(operation.DirectOutput).Kind == ShellTypeKind.Result
                ? EvalOutcome.Success(new ShellValue(operation.DirectOutput, new ShellResultValue.Success(sortedValue)))
                : EvalOutcome.Success(sortedValue);
        }
